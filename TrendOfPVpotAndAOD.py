...
ax1.set_ylim([0, 1.2])  # 根据你的图形，设置 AOD 的 y 轴范围

# 创建第二个 y 轴，用于绘制 PVpot
ax2 = ax1.twinx()
color = 'tab:orange'
ax2.set_ylabel('PV$_{POT}$', color=color)
line2, = ax2.plot(years, pvpot_values, 'o-', color=color, label='PV$_{POT}$')
ax2.tick_params(axis='y', labelcolor=color)
ax2.set_ylim([0.19, 0.23])  # 根据你的图形，设置 PVpot 的 y 轴范围

# 添加图例，并将图例分为两行显示
lines = [line1, line2]
labels = [line1.get_label(), line2.get_label()]
fig.legend(lines, labels, loc="upper center", bbox_to_anchor=(0.8, 1.0), ncol=2, bbox_transform=ax1.transAxes)
